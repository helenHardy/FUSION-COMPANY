-- FIX V5: MOTOR DE REGALÍAS Y CIERRE CONSOLIDADO PROFESIONAL (REPARADO)
-- Incluye todos los campos de depuración necesarios para evitar errores en el Frontend.

-- 1. ASEGURAR ESTRUCTURA
ALTER TABLE public.ranks ADD COLUMN IF NOT EXISTS personal_bonus_percentage NUMERIC(5, 2) DEFAULT 0;

-- 2. FUNCIÓN DE ESTADO DE REGALÍAS (CORREGIDA)
...
-- 3. MOTOR DE CIERRE MENSUAL (REPARADO)
    ...
    LOOP
        ...
        IF v_user.royalties_config IS NOT NULL THEN
            FOR v_milestone IN SELECT * FROM royalty_milestones ORDER BY level_number ASC LOOP
                IF v_user.monthly_pvg >= v_milestone.min_pvg AND v_user.monthly_pv >= v_milestone.min_monthly_pv THEN
